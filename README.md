...
This is a guide to building a simple lottery game, implemented as a market on Synthetix V3. The source code that results from this [is on GitHub]().
...
There will be two main methods in the smart contract for the market:

- `buyTicket()`: In exchange for the ticket price, this function gives the user a chance to win the jackpot at the next draw. The `recipient` parameter allows for one address to "gift" any address the lottery ticket.
...
- `payout()`: If a participant previously called `buyTicket` with the matching number, they win the jackpot. Otherwise, the jackpot is automatically distributed among liquidity providers by the Synthetix system.
...
To make the market profitable, the odds of any individual lottery ticket winning could be 1 in 1,000 and a fee that is always earned to liquidity providers could be added (as a `feePercent` parameter).
...
For this guide we will be building the market with [Foundry](). Even if you prefer to do the project with Hardhat, use of later tools requires Anvil from Foundry. If you haven't already,
you can install foundry to your machine:
...
As you can see, other than supportsInterface for ERC-165, we have to implement 3 functions. Lets go over these in detail:

- `name(uint128 marketId) returns (string memory)`: Returned to get a human-readable name for the market. This is useful for display on dashboards or when people are browsing markets registered with Synthetix. In the code above, we are rendering the ID as a bytes string, which isn't ideal. You could improve this code by relying on Open Zeppelin’s [string library](https://docs.openzeppelin.com/contracts/4.x/api/utils#Strings).
...
- `locked(uint128 marketId) returns (uint256)`: Similar to `reportedDebt`, this allows the market to control the minimum amount of liquidity provided to it via pools. Collateral cannot be withdrawn such that the remaining value is below the amount returned by this function. This is useful when a market may be about to accumulate a large amount of debt and liquidity providers are able to anticipate this and choose to leave. The lottery market uses this function to prevent withdrawals when a draw is in progress, as we will see later.
